...
const CustomDeleteDialog: React.FC<DialogComponentProps> = ({ open, handleClose, quantity, onAccept, onRefuse }) => {
  return (
    <Modal
      open={open}
    ...
    >
      <Box sx={{
        ...
        <DialogContent sx={{ padding: 0, display: 'flex', alignItems: 'center', height: '60px', border: '2px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)', borderRadius: '8px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', backgroundColor: '#1976d2', height: '100%', width: '60px', justifyContent: 'center', color: '#fff', borderRadius: '8px' }}>
            {quantity}
          </Box>
          ...
          <IconButton
            onClick={onAccept}
            sx={{ marginRight: '8px', height: '32px', color: 'green' }}
          >
            <CheckIcon />
          </IconButton>
          <IconButton
            onClick={onRefuse}
            sx={{ marginRight: '8px', height: '32px', color: 'red' }}
          >
            <DoNotDisturbAltIcon />
          </IconButton>
